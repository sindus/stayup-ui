...
export const zh: DocContent = {
  ...
  home: {
    ...
    concept: {
      heading: '四句话讲清楚',
      points: [
        'StayUp 会把你关注的来源的新内容展示给你——某个 GitHub 项目的发布、一个 YouTube 频道、一个 RSS 订阅源、一个网页。',
        '提供方是一个小程序，它去抓取某一类来源，并把抓到的内容写进一个 PostgreSQL 数据库。',
        'StayUp API 读取这个数据库，再把内容交给各个应用。它并不认识 YouTube，也不认识 RSS：它只是如实报告数据库里有什么。',
        '各个应用——网页、桌面、移动——读取 API。每一个都能指向任意实例，也就是任意数据库。',
      ],
      note: '整个设计就这些。下面的一切都只是细节。',
      diagram: {
        title: '从一个来源到你的屏幕',
        sources: '外部来源',
        sourcesItems: '一个 GitHub 仓库 · 一个 YouTube 频道 · 一个 RSS 订阅源 · 一个网页',
        providers: '提供方',
  ...
  providers: {
    ...
    eyebrow: '提供方',
    title: '接入一个新来源',
    lede: '提供方就是一个去抓取某类来源、并保存所得内容的程序。为了让它出现，StayUp 的其他部分不需要任何改动。',
    ...
    what: {
      ...
      diagram: {
        title: '一个 RSS 提供方，逐步来看',
        sources: '从数据库读到的、属于它的来源',
    ...
    existing: {
      heading: '先复用，再自己写',
      body: '已经有四个提供方：GitHub 发布、YouTube、RSS，以及抓取一个普通网页。每个都是可以指向你自己数据库的独立小仓库，也都是能跑起来的示例。RSS 那个最短，建议与本页对照阅读。',
    },
